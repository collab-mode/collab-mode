...
class MainListiningThread(threading.Thread):
	...
	def messageHandler(self,con,mess):
		if (mess.getBody() != None):
			mbody = str(mess.getBody()).replace("\\","\\\\").replace('"','\\"')
			mymess = '(:whisper "' +str(mess.getFrom()).split("/",1)[0] + '"  "' +  mbody + '")' 
	...
	def listmainaddr(self):
		sendmsg = '(:mainusers '
		for i in range(len(rooms[0][1])):
                        sendmsg = sendmsg + '(' + str(i) + ' ' + str(rooms[0][1][i].getpeername()).replace('(','',1).replace(',',' ',1).replace("'","\"").replace(')','')
			#if (unames[str(rooms[self.myroom][1][i].getpeername())][0] != ''):
			sendmsg = sendmsg + ' "' + str(unames[str(rooms[0][1][i].getpeername())][0]) + '"'
			sendmsg = sendmsg + " " + str(unames[str(rooms[0][1][i].getpeername())][1]).replace("(",'').replace("'",'').replace(',',' ').replace(')','')
			sendmsg = sendmsg + ')'
                sendmsg = sendmsg + ')'
...
def main():
	...
	canconn = True
		...
		try:
			...
			print "invalid ssl connection attempt"
			canconn = False
		if canconn:
